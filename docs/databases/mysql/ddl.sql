...
CREATE TABLE `license_user-role`
(
    ...
    `deleted_at`      DATETIME          DEFAULT NULL COMMENT '删除时间',
    PRIMARY KEY (`auto_id`),
    KEY               `idx_license_user-role_license_user_id` (`license_user_id`),
    KEY               `idx_license_user-role_role_id` (`role_id`),
    CONSTRAINT `fk_license_user-role_license_user_id` FOREIGN KEY (`license_user_id`) REFERENCES `license_user` (`auto_id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `fk_license_user-role_role_id` FOREIGN KEY (`role_id`) REFERENCES `role` (`auto_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='授权操作用户-角色';

CREATE TABLE `license_admin-role`
(
    `auto_id`          BIGINT   NOT NULL AUTO_INCREMENT COMMENT '用户角色ID',
    ...
    `deleted_at`       DATETIME          DEFAULT NULL COMMENT '删除时间',
    PRIMARY KEY (`auto_id`),
    KEY                `idx_license_admin-role_license_admin_id` (`license_admin_id`),
    KEY                `idx_license_admin-role_role_id` (`role_id`),
    CONSTRAINT `fk_license_admin-role_license_admin_id` FOREIGN KEY (`license_admin_id`) REFERENCES `license_admin` (`auto_id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `fk_license_admin-role_role_id` FOREIGN KEY (`role_id`) REFERENCES `role` (`auto_id`) ON DELETE CASCADE ON UPDATE CASCADE
